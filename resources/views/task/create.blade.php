<x-app-layout>
  <x-dashboard.form method="POST" action="{{ route('task.store', $board) }}">
    ...
    <select name="status" id="status"
      ...
      @foreach ($options as $option)
        <option {{ $option['value'] == '' ? 'selected' : '' }} value="{{ $option['value'] }}">{{ $option['label'] }}
        </option>
      @endforeach
    </select>

    <label for="status" class="block mb-2 text-sm font-medium text-gray-900">Deadline</label>
    <x-dashboard.date-picker name="deadline" value="" />
    ...
    @foreach ($assignable_teams as $assignable)
      <x-checkbox checked="" name="assigned_teams[]" label="{{ $assignable->name }}"
        value="{{ $assignable->id }}" />
    @endforeach

    <label class="block mb-2 text-m font-medium text-gray-900">Assign to members:</label>
    @foreach ($assignable_users as $assignable)
      <x-checkbox checked="" name="assigned_users[]" label="{{ $assignable->name }}"
        value="{{ $assignable->id }}" />
    @endforeach
